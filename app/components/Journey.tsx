...
const videoMemories: VideoMemory[] = [
  {
    ...
    poster: "linear-gradient(135deg, rgba(255,111,181,0.30), rgba(73,34,115,0.85))",
  },
  {
    title: "Quiet Laughs",
    year: "2025",
...
function GradientFallback({ caption, note, glow }: { caption: string; note: string; glow: string }) {
  return (
    <div
      className="absolute inset-0 flex flex-col justify-end overflow-hidden rounded-[2rem]"
      style={{
        background: `radial-gradient(circle at top left, ${glow}, transparent 42%), linear-gradient(135deg, rgba(20,10,30,0.95), rgba(67,21,94,0.85))`,
      }}
    >
      ...
      <div className="relative z-10 p-6 sm:p-7">
        ...
        <div className="mt-4 text-2xl font-semibold leading-tight text-white sm:text-3xl">{caption}</div>
        <div className="mt-3 max-w-md text-sm text-white/75 sm:text-base">{note}</div>
      </div>
    </div>
  );
}

function SlideFrame({ slide, broken, onImageError }: { slide: PhotoSlide; broken: boolean; onImageError: () => void }) {
  return (
    <div className="absolute inset-0 overflow-hidden rounded-[2rem]">
      {!broken && slide.src ? (
        <Image src={slide.src} alt={slide.caption} fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover" onError={onImageError} priority={false} />
      ) : (
        <GradientFallback caption={slide.caption} note={slide.note} glow={slide.accent} />
      )}
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      {Array.from({ length: 12 }).map((_, index) => {
        const x = (seed * 17 + index * 13) % 100;
        const y = (seed * 23 + index * 9) % 100;
        const size = 5 + (index % 3) * 2;
        return <span key={`${seed}-${index}`} className="sparkle" style={{ left: `${x}%`, top: `${y}%`, width: size, height: size, animationDelay: `${index * 0.12}s` }} />;
      })}
...
export default function Journey() {
  ...
  const [photoErrors, setPhotoErrors] = useState<boolean[]>(() => gallerySlides.map(() => false));
  ...
  useEffect(() => {
    ...
    const sequence = window.setTimeout(() => {
      setShowLoveNote(false);
    }, 6500);

    return () => window.clearTimeout(sequence);
  }, [showLoveNote]);
  ...
  const fallbackPhoto = (index: number) => {
    setPhotoErrors((value) => value.map((item, itemIndex) => (itemIndex === index ? true : item)));
  };
  ...
  return (
    <div ref={rootRef} className="relative min-h-screen overflow-hidden bg-[#140716] text-white">
      <div id="cursor-glow" className="pointer-events-none fixed left-0 top-0 z-40 h-80 w-80 rounded-full bg-[radial-gradient(circle,rgba(255,111,181,0.22),rgba(201,182,255,0.14),transparent_70%)] blur-3xl mix-blend-screen" />
      ...
      <main className="relative z-10">
            ...
            <div className="flex-1" data-reveal>
              ...
              <div className="mt-8 max-w-3xl space-y-5">
                ...
                <p className="max-w-2xl text-base leading-8 text-white/75 sm:text-lg">
                  Out of billions of people, somehow you became my favorite place, favorite smile, favorite everything. This is not a webpage. It is a journey through the moments, the love, and the future I keep imagining with you.
                </p>
              ...
              <div ref={heroCardRef} className="relative mx-auto w-full max-w-2xl rounded-[2.25rem] border border-white/10 bg-white/5 p-4 shadow-[0_30px_120px_rgba(0,0,0,0.55)] backdrop-blur-2xl transform-gpu">
                <div className="relative aspect-[4/5] overflow-hidden rounded-[1.85rem]">
                  <AnimatePresence mode="wait">
                    ...
                    >
                      <SlideFrame slide={activeSlide} broken={photoErrors[currentSlide]} onImageError={() => fallbackPhoto(currentSlide)} />
                    </motion.div>
                  </AnimatePresence>
                  ...
                  <div className="pointer-events-none absolute -inset-8 -z-10 rounded-[3rem] opacity-70 blur-3xl transition-all duration-1000" style={{ boxShadow: `0 0 180px 50px ${activeSlide.accent}` }} />
                ...
                <motion.button key={memory.title} onClick={() => setActiveVideo(index)} whileHover={{ y: -10, rotate: index % 2 === 0 ? 1 : -1 }} className="group relative overflow-hidden rounded-[1.9rem] border border-white/10 bg-white/5 text-left shadow-[0_30px_90px_rgba(0,0,0,0.45)] backdrop-blur-xl">
                  <div className="relative aspect-[16/10] overflow-hidden">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <motion.div animate={{ scale: [1, 1.08, 1] }} transition={{ duration: 2.4, repeat: Infinity, ease: "easeInOut" }} className="grid h-18 w-18 place-items-center rounded-full border border-white/30 bg-white/10 text-2xl text-white shadow-[0_0_30px_rgba(255,255,255,0.15)]">
                        ▶
                      </motion.div>
                    </div>
            ...
            <div className="relative mt-12 overflow-hidden rounded-[2.25rem] border border-white/10 bg-white/5 p-8 backdrop-blur-2xl">
              ...
              <div className="relative flex min-h-[420px] flex-wrap items-center justify-center gap-5 sm:gap-8">
                {wishWords.map((word, index) => (
                  <motion.span
                    key={word}
                    initial={{ opacity: 0, y: 40, rotate: index % 2 === 0 ? -8 : 8 }}
                    whileInView={{ opacity: 1, y: 0, rotate: index % 2 === 0 ? 4 : -4 }}
                    viewport={{ once: false, amount: 0.6 }}
                    transition={{ duration: 0.9, delay: index * 0.08 }}
                    className={`text-4xl font-semibold tracking-[-0.06em] sm:text-6xl lg:text-7xl ${index % 3 === 0 ? "text-white" : index % 3 === 1 ? "text-[#ffd6e8]" : "text-[#c9b6ff]"}`}
                  >
                    {word}
                  </motion.span>
                ))}
        ...
        <section className="relative min-h-screen px-6 pb-28 pt-24 sm:px-10 lg:px-16">
          ...
          <div className="relative mx-auto flex min-h-[760px] max-w-6xl flex-col items-center justify-center text-center">
            ...
            <motion.h2 animate={{ opacity: [0.8, 1, 0.8] }} transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }} className="mt-6 text-5xl font-semibold tracking-[-0.06em] text-white sm:text-7xl lg:text-[6.75rem]">
              Kamal <span className="text-[#ff8fc8]">❤️</span> Tiara
            </motion.h2>
            ...
            <motion.div initial={{ y: 18, scale: 0.98 }} animate={{ y: 0, scale: 1 }} className="relative z-10 max-w-xl rounded-[2rem] border border-white/10 bg-[#fffaf6] p-8 text-black shadow-[0_30px_120px_rgba(0,0,0,0.45)]">
              ...
              <AnimatePresence mode="wait">
                <motion.p key={surpriseStep} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="mt-5 text-lg leading-8 text-black/75">
                  {surpriseLines[surpriseStep] === "Tiara"
                    ? "Tiara, you make ordinary days feel special. Thank you for existing. Happy Birthday, my love. — Kamal ❤️"
                    : surpriseLines[surpriseStep]}
                </motion.p>
              </AnimatePresence>
